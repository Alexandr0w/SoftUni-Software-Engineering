...
namespace DI.Core
{
    public abstract class AbstractModule : IModule
    {
        ...
        public object? GetService(Type serviceType) => this.GetService(serviceType, new HashSet<Type>());
        ...
        protected void RegisterService(Type inferaceType, Type implementationType)
        {
            ...
            if (!implementationType.IsAssignableTo(inferaceType))
            {
                throw new InvalidOperationException("Implementation type does not inherit the provided service type.");
            }

            if (this._serviceTypesMap.ContainsKey(inferaceType))
            {
                throw new InvalidOperationException($"This module already has a registration for service of type {inferaceType.Name}");
            }

            this._serviceTypesMap[inferaceType] = implementationType;
        }
        ...
        private object? Instantiate(Type implementationType, HashSet<Type> cycleDetector)
        {
            ...
            object?[] dependecies = new object?[parameters.Length];

            for (int i = 0; i < parameters.Length; i++)
            {
                dependecies[i] = this.GetService(parameters[i].ParameterType, cycleDetector);
            }

            object? result = constructor.Invoke(dependecies);
        ...
        private void InjectProperties(Type impementationType, object instance, HashSet<Type> cycleDetector)
        {
            PropertyInfo[] properties = impementationType.GetProperties(MemberSearchBindingFlags);
