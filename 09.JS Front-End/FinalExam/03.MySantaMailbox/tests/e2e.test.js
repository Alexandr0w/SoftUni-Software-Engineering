...
describe('E2E tests', function () {
  ...
  describe('My Santa Mailbox Tests', () => {
    it('Load Presents', async () => {
      ...
      const list = await page.$$eval(`#presents #gift-list .gift-box`, (t) =>
   ...
   it('Add Gift', async () => {
      ...
      await page.fill('#for-whom', data.forWhom);
      ...
      expect(postData.forWhom).to.equal(data.forWhom);
      expect(postData.price).to.equal(data.price);

      const [gift] = await page.$$eval(`#gift`, (t) =>
        t.map((s) => s.value)
      );
      const [forGift] = await page.$$eval(`#for-whom`, (t) =>
        t.map((s) => s.value)
      );
    ...
    it('Edit Gift (Has Input)', async () => {
      ...
      await page.click('#gift-list .gift-box .change-btn');
      ...
      expect(allCourse[1]).to.include(data.forWhom);
